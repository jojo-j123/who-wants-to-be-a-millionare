...
function createRouter(host) {
  ...
  async function handle(req, res, url, pathname) {
    ...
    // The stage display is often a TV with no keyboard, so the logo is open in
    // the same way the state is. It is cached hard and addressed by content
    // hash (?v=), so a screen fetches each logo exactly once however long the
    // show runs.
    if (pathname === '/api/logo' && (req.method === 'GET' || req.method === 'HEAD')) {
      const logo = host.readLogo ? await host.readLogo() : null;
      if (!logo) return json(res, 404, { error: 'No logo has been uploaded.' });
    ...
    // Uploading stores the image under its own key and records only the hash in
    // settings, which is what tells every screen to go and fetch the new one.
    if (pathname === '/api/logo' && (req.method === 'POST' || req.method === 'PUT')) {
      requireWritable(host);
      if (!host.writeLogo) return json(res, 501, { error: 'This deployment cannot store a logo.' });
      const logo = api.sanitizeLogo(await readJson(req));
      await host.writeLogo(logo);
      ctx.settings.display.logoVersion = logo.version;
      await host.persist(ctx, { settings: true });
      host.broadcast(ctx);
      return json(res, 200, { ok: true, version: logo.version, bytes: logo.bytes, mime: logo.mime });
    }

    if (pathname === '/api/logo' && req.method === 'DELETE') {
      requireWritable(host);
      if (host.writeLogo) await host.writeLogo(null);
      ctx.settings.display.logoVersion = '';
      await host.persist(ctx, { settings: true });
      host.broadcast(ctx);
